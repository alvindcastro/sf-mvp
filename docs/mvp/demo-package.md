...
| 0:25-0:55 | `docs/mvp/scope.md` | "The trust boundary is explicit: synthetic data only, no live fleet integrations, no autonomous export or escalation, and no production compliance claims." |
...
| 2:45-3:25 | `docs/mvp/eval-plan.md` and `internal/eval` tests | "The repo includes deterministic golden eval cases for severity, citation coverage, recommendation accuracy, unsupported claims, redaction, prompt-injection resistance, and approval fail-closed behavior." |
| 3:25-4:10 | `docs/mvp/observability-and-cost-controls.md` and `internal/observability` tests | "The observability package records in-memory trace, retrieval, tool-call, approval, latency, token, budget, and eval events. It also defines cache candidates and model-routing notes, but it does not call a live provider or reconcile billing." |
